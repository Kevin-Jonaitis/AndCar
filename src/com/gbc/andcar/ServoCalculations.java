...
class ServoCalculations {
	...
	public static final int FASTFORWARDMOTOR = 1650;
	public static final int FORWARDMOTOR = 1625;
	public static final int BACKMOTOR = 1300;

	public static final int FORWARDSTOP = 1550;
	public static final int BACKSTOP = 1450;
	...
	public void calculateWheelPW(double currentMaxArea) {
		double diffC = 1.2; // the difference constant
	...
	public class IRCalculations {
		...
		public void calculateIR(boolean doBacking, boolean reverseWheels) {
			if (leftIRVoltage > 2.0 || rightIRVoltage > 2.0 ) {
				doBacking = true;
			...
			if (doBacking) {
				...
				if (leftIRVoltage < 1.0 || rightIRVoltage < 1.0) {
					doBacking = false;
					reverseWheels = false;
				}
			}
		...
		public void calculateSideIR() {
			...
			double diffC = 0.33;
			int difference = (WHEELMAX - WHEELMIN);
			double beginTurnVoltage = 1.5; // when the wheels just begin to turn
			double maxTurnVoltage = 2.0; // when the wheels should be turning at
											// maximum

			double changeInPWLeft = diffC * (lSideVoltage / maxTurnVoltage)
					* difference;
			double changeInPWRight = diffC * (rSideVoltage / maxTurnVoltage)
					* difference;
			...
			public void caculateLeftRightIR()
		...
		
			{
				...
				int difference = WHEELMAX - WHEELMIN;
				double beginTurnVoltage = 1.0; //when the wheels just begin to turn
				double maxTurnVoltage = 2.0; //when the wheels should be turning at maximum
				
				double changeInPWLeft = (leftIRVoltage/maxTurnVoltage) * difference;
				double changeInPWRight = (rightIRVoltage/maxTurnVoltage) * difference;
